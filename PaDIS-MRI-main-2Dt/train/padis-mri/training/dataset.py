...
class ImageFolderDatasetX(Dataset):
    def __init__(self,
        ...
        slice_tensor = data["x_est_gt"]  # (N, T, H, W) complex
        
        # (N, T, H, W, 2) -> (N, T, 2, H, W) -> (N, 2*T, H, W)
        ri = torch.view_as_real(slice_tensor).to(torch.float32).permute(0, 1, 4, 2, 3).contiguous()
        real_imag_tensor = ri.view(ri.shape[0], -1, ri.shape[3], ri.shape[4]).cpu().numpy()

...
def denoisedFromPatches(net, x, t_hat, latents_pos, class_labels, indices, pad=(0, 0), t_goal = -1, avg=1, spaced=[], wrong=False):
    ...
    pos_ch = latents_pos.shape[1]   # <-- key change (2 for 2D, 2+t_patch for cine)
    
    x_input = torch.zeros((patches, channels, psize, psize), device=x_hat.device, dtype=x_hat.dtype)
    pos_input = torch.zeros((patches, pos_ch, psize, psize), device=latents_pos.device, dtype=latents_pos.dtype)
    
    for i in range(patches):
        z = indices[i]
        x_input[i] = x_hat[0, :, z[0]:z[1], z[2]:z[3]]
        pos_input[i] = latents_pos[0, :, z[0]:z[1], z[2]:z[3]]

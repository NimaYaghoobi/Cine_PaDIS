...
class DPSHyperEvaluator:
    def __init__(self,
        ...
        image_size: size of GT images (e.g. (H,W))
        ...
        # self.pad = pad
        self.pad = tuple(pad)
        self.pad_y, self.pad_x = int(self.pad[0]), int(self.pad[1])

        self.psize = psize
        self.mask_select = mask_select
        # self.inverseop = InverseOperator(self.image_size)
        self.inverseop = InverseOperator((self.H, self.W))
    ...
    def _build_latents_and_pos(self, pad_override: Tuple[int, int]=None)-> None:
        ...
        # self.latents = torch.randn([1, 1, self.image_size, self.image_size],device=self.device)
        self.latents = torch.randn([1, 1, self.H, self.W], device=self.device)
        ...
        xs = torch.linspace(-1, 1, Wp, device=self.device)
        ys = torch.linspace(-1, 1, Hp, device=self.device)
    ...
    def dps2_wrapper(self, 
        ...
        if measurement is None:
            ...
            latents = torch.randn([1, 1, H, W], device=clean.device)
            
            # Hp = H + 2*pad
            # Wp = W + 2*pad
            # xs = torch.linspace(-1, 1, Wp, device=clean.device)
            # ys = torch.linspace(-1, 1, Hp, device=clean.device)
            # x_pos = xs.view(1, -1).repeat(Hp, 1)
            # y_pos = ys.view(-1, 1).repeat(1, Wp)
